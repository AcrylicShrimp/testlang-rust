...
define_ast_node!(ASTFunctionNode {
    name: String,
    return_ty: Option<ASTType>,
    parameter_vec: Vec<(String, Option<ASTType>)>,
    body_ast_block: ASTBlockNode,
});
...
fn from_raw_ast_node(raw_ast: &RawAST) -> ASTNode {
    match raw_ast.children[0].name.as_ref() {
        ...
        "func-def-statement" => ASTNode::Function(from_raw_ast_node_function(&raw_ast.children[0])),
        "expression" => ASTNode::Expression(from_raw_ast_node_expression(&raw_ast.children[0])),
        _ => unreachable!("unknown ast: {}", raw_ast.children[0].name),
    }
}
...
fn from_raw_ast_node_function(raw_ast: &RawAST) -> ASTFunctionNode {
    ASTFunctionNode {
        name: raw_ast.children[1]
            .child
            .as_ref()
            .unwrap()
            .token_content
            .clone(),
        return_ty: match raw_ast.children.len() {
            5 => None,
            6 => {
                if raw_ast.children[4].name == "type" {
                    Some(
                        match raw_ast.children[4].children[0]
                            .child
                            .as_ref()
                            .unwrap()
                            .token_type
            ...
            7 => Some(
                match raw_ast.children[5].children[0]
                    .child
                    .as_ref()
                    .unwrap()
                    .token_type
        ...
        parameter_vec: {
            if raw_ast.children[3].name != "func-def-statement-param-list" {
                Vec::new()
            } else {
                let mut raw_ast_node_stack = raw_ast.children[3]
                    .children
                    .iter()
                    .rev()
                    .collect::<Vec<&RawAST>>();
                ...
                while match raw_ast_node_stack.last() {
                    Some(last_raw_ast) => last_raw_ast.name == "func-def-statement-param-list",
                    None => false,
                } {
                    let raw_ast_node_stack_tail = &mut raw_ast_node_stack
                        .pop()
                ...
                for ast in raw_ast_node_stack
                    .into_iter()
                    .rev()
                    .filter(|raw_ast| raw_ast.name != "Comma")
                {
                    if ast.name == "Id" {
                        id_vec.push((ast.child.as_ref().unwrap().token_content.clone(), None));
                    } else {
                        id_vec.last_mut().unwrap().1 =
                            Some(match ast.children[0].child.as_ref().unwrap().token_type {
                                TokenType::KeywordVoid => ASTType::Void,
                                TokenType::KeywordBool => ASTType::Bool,
                                TokenType::KeywordI8 => ASTType::I8,
                                TokenType::KeywordI16 => ASTType::I16,
        ...
        body_ast_block: match raw_ast.children.len() {
            5 => from_raw_ast_node_block(&raw_ast.children[4]),
            6 => from_raw_ast_node_block(&raw_ast.children[5]),
            7 => from_raw_ast_node_block(&raw_ast.children[6]),
            _ => unreachable!(),
        },
    }
}
...
fn from_raw_ast_node_cast(raw_ast: &RawAST) -> ASTCastNode {
    ASTCastNode {
        lhs: Box::new(from_raw_ast_node_expression(&raw_ast.children[0])),
        ty: match raw_ast.children[2].children[0]
            .child
            .as_ref()
            .unwrap()
            .token_type
